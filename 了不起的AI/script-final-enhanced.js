...
class UltimateSmartChatbot {
    constructor() {
        ...
        this.isMobileDevice = this.touchSupport && (this.isIOS || this.isAndroid);
    ...
    adjustLayout() {
        ...
        const isTablet = width <= 900 && width > 768;
        const isLandscape = height < 500 && width > height;

        // 移动端优化
        if (isMobile) {
            this.elements.messageContainer.style.maxHeight = 'calc(100vh - 220px)';
            this.elements.messageContainer.style.overflowY = 'auto';
            ...
            // 横屏模式优化
            if (isLandscape) {
                this.elements.messageContainer.style.maxHeight = 'calc(100vh - 180px)';
            }
        } else if (isTablet) {
            this.elements.messageContainer.style.maxHeight = 'calc(100vh - 240px)';
        } else {
        ...
        // 触发设备变化事件
        this.triggerEvent('deviceChange', { deviceType: this.deviceType, width, height });
    }
    ...
    initTouchEvents() {
        ...
        messageContainer.addEventListener('touchmove', (e) => {
            ...
            if (Math.abs(deltaX) > this.touchState.swipeThreshold ||
                Math.abs(deltaY) > this.touchState.swipeThreshold) {
        ...
        messageContainer.addEventListener('touchend', (e) => {
            ...
            if (!this.touchState.isSwiping && Date.now() - this.touchState.startTime < 200) {
                // 轻触 - 隐藏键盘（如果是输入区域外）
        ...
        console.log('触摸事件初始化完成，设备支持:', this.touchSupport ? '是' : '否');
    }
    ...
    handleLongPress(x, y) {
        ...
        menu.innerHTML = `
            <button class="menu-item" data-action="copy">复制</button>
            <button class="menu-item" data-action="share">分享</button>
        ...
        menu.addEventListener('click', (e) => {
            ...
            this.handleTouchMenuAction(action);
            menu.remove();
        });
    ...
    initMobileInputOptimization() {
        ...
        if (window.visualViewport) {
            window.visualViewport.addEventListener('resize', () => {
                this.debounce('viewportResize', () => {
                    this.handleViewportResize();
                }, 100);
            });
        }

        console.log('移动端输入优化完成，设备:', this.isIOS ? 'iOS' : this.isAndroid ? 'Android' : '其他');
    }
    ...
    scrollInputIntoView() {
        ...
        if (inputRect.bottom > viewportHeight - 100) {
            const scrollY = inputRect.bottom - viewportHeight + 150;
            window.scrollTo({
    ...
    handleViewportResize() {
        ...
        const isKeyboardVisible = viewport.height < window.innerHeight * 0.7;

        if (isKeyboardVisible) {
            // 键盘弹出时调整布局
            this.elements.messageContainer.style.maxHeight = `calc(${viewport.height}px - 250px)`;
