...
class SmartHealingChatbot {
    constructor() {
        ...
        this.conversationContext = {
            ...
            userMood: 'neutral',         // 用户情绪: neutral, happy, sad, angry, tired
            lastTopics: [],              // 最近话题历史
            mentionedPreferences: new Set(), // 用户提到的偏好
            mentionedPeople: new Set(),  // 用户提到的人名
            mentionedActivities: new Set(), // 用户提到的活动
            userInterests: new Set(),    // 用户兴趣
            conversationDepth: 0,        // 对话深度
            needsFollowUp: false,        // 需要跟进
            followUpQuestion: null       // 跟进问题
        };
        ...
        this.userProfile = {
            ...
            age: null,                   // 用户年龄
            location: null,              // 用户位置
            occupation: null,            // 用户职业
            hobbies: [],                 // 用户爱好
            recentEvents: [],            // 最近事件
            emotionalState: 'neutral',   // 情绪状态
            lastActive: new Date()       // 最后活跃时间
        };

        // 对话记忆（最近10轮对话）
        this.shortTermMemory = [];
        this.shortTermMemoryLimit = 10;
    ...
    addWelcomeMessage() {
        const welcomeMessages = [
            ...
            "我升级了智能系统，现在能更好地理解你、记住我们的对话，并提供更有用的信息。🌸",
            "你可以和我聊任何话题～心情、生活、兴趣，或者只是随便聊聊。我都在这里陪伴你！✨"
        ];
    ...
    sendMessage() {
        ...
        setTimeout(() => {
            ...
            // 更新对话记忆
            this.updateConversationMemory(message, aiResponse);
        }, 800 + Math.random() * 800);
    }
    ...
    analyzeUserMessage(message) {
        ...
        // 情感分析
        this.analyzeEmotion(lowerMsg);

        // 提取用户信息
        this.extractUserInfo(message);

        // 识别话题
        this.identifyTopic(lowerMsg);

        // 提取偏好和兴趣
        this.extractPreferences(lowerMsg);
    ...
    analyzeEmotion(message) {
        ...
        if (detectedEmotions.length > 0) {
            // 取第一个检测到的情绪作为主要情绪
            this.conversationContext.userMood = detectedEmotions[0];
            this.userProfile.emotionalState = detectedEmotions[0];
        }

        // 特别处理强烈情绪词
        if (message.includes('非常') || message.includes('特别') || message.includes('极度')) {
    ...
    extractUserInfo(message) {
        // 简单提取姓名（如果消息包含"我叫"、"我是"等）
        const namePatterns = [/我叫(.{2,4})/, /我是(.{2,4})/, /name is (.{2,10})/i];
        ...
        // 提取年龄
        const agePattern = /(\d{1,2})岁|年龄(\d{1,2})|今年(\d{1,2})/;
        const ageMatch = message.match(agePattern);
        if (ageMatch) {
            for (let i = 1; i <= 3; i++) {
                if (ageMatch[i] && !isNaN(ageMatch[i])) {
                    this.userProfile.age = parseInt(ageMatch[i]);
                    break;
                }
            }
        }
        ...
        // 提取地点
        const locationKeywords = ['住在', '在', '来自', 'location', 'city'];
        if (locationKeywords.some(keyword => message.includes(keyword))) {
            // 简单提取：假设地名在关键词后面
            for (const keyword of locationKeywords) {
                const idx = message.indexOf(keyword);
                if (idx !== -1) {
                    const after = message.substring(idx + keyword.length, idx + keyword.length + 10);
                    const words = after.split(/[，。！？\s]/);
                    if (words[0] && words[0].length > 0) {
                        this.userProfile.location = words[0].trim();
                        break;
                    }
                }
            }
    ...
    identifyTopic(message) {
        const topicCategories = {
            work: ['工作', '上班', '同事', '老板', '项目', '任务', '职场', '职业'],
            study: ['学习', '考试', '作业', '学校', '老师', '同学', '课程', '读书'],
            family: ['家人', '父母', '妈妈', '爸爸', '孩子', '家庭', '亲戚'],
            friends: ['朋友', '闺蜜', '兄弟', '伙伴', '聚会', '社交'],
            hobby: ['爱好', '兴趣', '游戏', '运动', '音乐', '电影', '阅读', '旅行'],
            health: ['健康', '身体', '生病', '医院', '医生', '锻炼', '饮食'],
            love: ['恋爱', '喜欢', '爱情', '男朋友', '女朋友', '约会', '分手'],
            daily: ['今天', '昨天', '最近', '日常', '生活', '计划']
        };
        ...
        if (detectedTopics.length > 0) {
            this.conversationContext.currentTopic = detectedTopics[0];
            // 添加到话题历史
            if (!this.conversationContext.lastTopics.includes(detectedTopics[0])) {
                this.conversationContext.lastTopics.unshift(detectedTopics[0]);
    ...
    extractPreferences(message) {
        const preferencePatterns = [
            { pattern: /喜欢(.{2,8})/, type: 'interest' },
            { pattern: /不喜欢(.{2,8})/, type: 'dislike' },
            { pattern: /爱(.{2,8})/, type: 'love' },
            { pattern: /讨厌(.{2,8})/, type: 'hate' }
        ];
        ...
        for (const { pattern, type } of preferencePatterns) {
            ...
            if (match && match[1]) {
                ...
                if (type === 'interest' || type === 'love') {
                    this.conversationContext.userInterests.add(item);
                    this.userProfile.hobbies.push(item);
                }
                this.conversationContext.mentionedPreferences.add(`${type}:${item}`);
            }
        }

        // 提取活动
        const activityKeywords = ['看电影', '听音乐', '打球', '跑步', '游泳', '旅游', '逛街', '做饭'];
        for (const activity of activityKeywords) {
            if (message.includes(activity)) {
                this.conversationContext.mentionedActivities.add(activity);
            }
        }
    }
    ...
    generateSmartResponse(userMessage) {
        ...
        if (this.conversationContext.needsFollowUp && this.conversationContext.followUpQuestion) {
            response = this.generateFollowUpResponse(lowerMsg);
            if (response) {
        ...
        if (this.conversationContext.followUpQuestion) {
            response = this.acknowledgeAnswer(lowerMsg);
            if (response) {
                const followUp = this.generateFollowUpQuestion();
                return this.processResponse(response + " " + followUp);
            }
        }

        // 3. 检查特定请求（如笑话、故事等）- 最高优先级
        const specificRequest = this.checkSpecificRequests(lowerMsg);
        if (specificRequest) {
            return this.processResponse(specificRequest);
        }

        // 4. 检查是否询问知识
        if (this.isKnowledgeQuery(lowerMsg)) {
            response = this.generateKnowledgeResponse(lowerMsg);
            if (response) return this.processResponse(response);
        }

        // 5. 检查是否寻求建议
        if (this.isAdviceQuery(lowerMsg)) {
            response = this.generateAdviceResponse(lowerMsg);
            if (response) return this.processResponse(response);
        }

        // 6. 根据用户情绪回应
        if (this.conversationContext.userMood !== 'neutral') {
            response = this.generateEmotionalResponse(lowerMsg);
            if (response) return this.processResponse(response);
        }

        // 7. 根据话题回应
        if (this.conversationContext.currentTopic) {
            response = this.generateTopicResponse(lowerMsg);
            if (response) return this.processResponse(response);
        }

        // 8. 个性化回复（使用用户信息）
        response = this.generatePersonalizedResponse(lowerMsg);
        if (response) return this.processResponse(response);
    ...
    // 生成情绪回应
    generateEmotionalResponse(message) {
        const { userMood } = this.conversationContext;
        const responses = {
            happy: [
                "看到你这么开心，我也感到很快乐！🎉",
                "真为你感到高兴！愿这份快乐一直陪伴你。😊",
        ...
        if (responses[userMood]) {
            return this.getRandomResponse(responses[userMood]);
        }
        return null;
    }

    // 生成话题回应
    generateTopicResponse(message) {
        const { currentTopic, lastTopics } = this.conversationContext;
        const topicResponses = {
            ...
            daily: [
                "日常生活虽然平凡，但也有很多小确幸呢。☕",
                "把每一天都过得充实而有意义，就是最好的生活。🌞",
                "生活中的小细节往往藏着大幸福。🌸"
            ]
        };

        if (topicResponses[currentTopic]) {
            let response = this.getRandomResponse(topicResponses[currentTopic]);
    ...
    isKnowledgeQuery(message) {
        const knowledgeKeywords = ['知道', '了解', '什么是', '为什么', '怎么', '如何', '原因', '科普', '笑话', '故事', '讲个'];
        return knowledgeKeywords.some(keyword => message.includes(keyword));
    }

    // 生成知识回复
    generateKnowledgeResponse(message) {
        ...
        const facts = this.knowledgeBase.facts[randomType];
        ...
        if (facts && facts.length > 0) {
            ...
            if (Math.random() > 0.5) {
                ...
                const randomAdviceType = adviceTypes[Math.floor(Math.random() * adviceTypes.length)];
                const adviceList = this.knowledgeBase.advice[randomAdviceType];
                if (adviceList && adviceList.length > 0) {
                    response += " " + this.getRandomResponse(adviceList);
                }
            }
    ...
    // 判断是否为建议查询
    isAdviceQuery(message) {
        const adviceKeywords = ['建议', '怎么办', '应该', '需要', '帮助', '解决', '方法', '技巧'];
        return adviceKeywords.some(keyword => message.includes(keyword));
    }

    // 生成建议回复
    generateAdviceResponse(message) {
        const adviceTypes = Object.keys(this.knowledgeBase.advice);
        const contextAdvice = {
            work: 'stress',
            study: 'study',
        ...
        if (this.conversationContext.currentTopic && contextAdvice[this.conversationContext.currentTopic]) {
            adviceType = contextAdvice[this.conversationContext.currentTopic];
        } else {
    ...
    generateFollowUpResponse(message) {
        if (this.conversationContext.followUpQuestion) {
            ...
            const isPositive = positiveKeywords.some(keyword => message.includes(keyword));
            const isNegative = negativeKeywords.some(keyword => message.includes(keyword));
    ...
    // 确认回答
    acknowledgeAnswer(message) {
        if (this.conversationContext.followUpQuestion) {
            return "谢谢你分享这些，这让我更了解你了。💕";
        }
        return null;
    }
    ...
    showHelp() {
        const helpMessage = `
            <strong>智能AI聊天伙伴使用说明：</strong><br>
            1. 我可以记住我们的对话内容和你的偏好<br>
            2. 我能识别你的情绪并给予恰当回应<br>
            3. 我拥有知识库，可以回答各种问题<br>
            4. 我会根据上下文进行连贯对话<br>
...
document.addEventListener('DOMContentLoaded', () => {
    const chatbot = new SmartHealingChatbot();
